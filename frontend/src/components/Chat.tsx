...
export function Chat() {
  ...
  const handleSend = () => {
    ...
    if (!text) return
    try {
      // En modos de red el chat viaja a través del peer; en solo se emite localmente
      const isNetwork = mode?.startsWith('lan') || mode?.startsWith('online')
      if (isNetwork) w.lanSendChat(text)
    ...
    <div className="flex flex-col h-full bg-slate-950/50">
      ...
      <div className="flex-1 overflow-y-auto p-2.5 flex flex-col gap-1 min-h-0">
        ...
        {messages.map(msg =>
          msg.kind === 'event' ? (
            <div key={msg.id}
              className="text-[11px] text-slate-500 italic px-1 py-0.5 leading-snug">
              {msg.content}
            </div>
          ) : (
            <div key={msg.id}
              className="text-[11px] bg-slate-800/60 rounded-lg px-2.5 py-1.5 leading-snug">
